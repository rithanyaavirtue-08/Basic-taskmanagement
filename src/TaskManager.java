    ...
    public void update(){
        while(!yes){
            try{
                ...
                if (upt < currentUser.getTaskArray().size() )
                {
                    String Status=currentUser.getTaskArray().get(upt-1).getStatus();
                    String description=currentUser.getTaskArray().get(upt-1).getDescription();
                    ...
                    if (cn == 1) {
                        if (upt < currentUser.getTaskArray().size()) {
                            ...
                            currentUser.getTaskArray().set(upt - 1, upName);
                        ...
                        while (true) {
                            ...
                            if (status == 1) {
                                currentUser.getTaskArray().get(upt - 1).setStatus("To Do");
                                System.out.println("Status Updated!");
                                break;
                            }
                            else if (status == 2) {
                                Task IP=currentUser.getTaskArray().get(upt - 1);
                                IP.setStatus("In Progress");
                                LocalDateTime DT=LocalDateTime.now();
                               // DateTimeFormatter formate=DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm");

                                IP.setStartDateTime(String.valueOf(DT));
                                currentUser.getTaskArray().get(taskCnt).setStartDateTime(String.valueOf(DT));
                                System.out.println("Status Updated!");
                                break;
                            }
                            else if (status == 3) {
                                currentUser.getTaskArray().get(upt - 1).setStatus("Progress Done");
                                LocalDateTime DT=LocalDateTime.now();
                                currentUser.getTaskArray().get(taskCnt).setStartDateTime(String.valueOf(DT));
                                System.out.println("Status Updated!");
                                break;
                        ...
                        String des = sc.nextLine();
                        currentUser.getTaskArray().get(upt - 1).setDescription(des);
